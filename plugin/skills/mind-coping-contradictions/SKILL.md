...
As CONTEXT + MEMORY grow, new data will contradict what is already stored. Contradiction is not a bug to delete on sight; it is data about where the model of the world is still wrong. This skill is the discipline of carrying contradictions safely until they can be properly reconciled.

## When to Use
- New information conflicts with an existing memory entry or stated belief.
- Two memory entries disagree.
- You feel the pull to immediately "pick a side" to relieve the discomfort.

## Core Principles

1. **Hold both, consciously.** Do not force a premature collapse to one side. A wrong fast resolution is worse than an honest held tension.
...
1. Detect the conflict and name both items plus their confidence/provenance.
2. Test for context-dependence: is there a condition under which each is true? If yes, encode that condition; the contradiction dissolves.
3. If genuinely contradictory, tag both IN-TENSION and route to audit: `mind-meditation` for low-stakes internal reconciliation, `mind-debate`/`mind-discuss` for anything high-stakes or where internal audit could just rationalize.
4. Do not edit memory to "fix" it until an audit justifies a confidence change (see `mind-meditation`).
